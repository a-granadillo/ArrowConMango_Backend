import { GetCube3DLeaderboardUseCase } from '../../../src/application/use-cases/get-cube3d-leaderboard.use-case';
...
const makeLbRepo = (entries: ScoreEntry[]): ILeaderboardRepository => ({
  ...
  byCube3d: jest.fn().mockResolvedValue(entries),
  add: jest.fn().mockResolvedValue(undefined),
});

describe('GetCube3DLeaderboardUseCase', () => {
  it('should_return_cube3d_entries_ranked_by_score_descending', async () => {
    // Arrange
    const slow = ScoreEntry.create(
      UserId.create('slow'),
      LevelId.create('cube-gen-1'),
      Score.create(3, 60_000),
      GameMode.cube3d(),
    );
    const fast = ScoreEntry.create(
      UserId.create('fast'),
      LevelId.create('cube-gen-2'),
      Score.create(1, 2_000),
      GameMode.cube3d(),
    );
    const repo = makeLbRepo([slow, fast]);
    const useCase = new GetCube3DLeaderboardUseCase(repo, strategy);
    // Act
    ...
    expect(repo.byCube3d).toHaveBeenCalledTimes(1);
  });
